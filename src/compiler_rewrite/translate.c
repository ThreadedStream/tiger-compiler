...
Tr_level Tr_outermost(void) {
    if (outermost_level == NULL) {
        ...
        outermost_level->frame = F_newFrame(Temp_namedlabel("tigermain"), NULL);
    }
...
Tr_exp Tr_simpleVar(Tr_access a, Tr_level l) {
    ...
    while (a->level != l) {
        staticLinkExp = F_upperStaticLinkExp(staticLinkExp);
        l = l->parent;
    }
    return Tr_Ex(F_ExpWithStaticLink(a->access, staticLinkExp));
}
...
Tr_exp Tr_callExp(bool isLibFunc,
    ...
    const char * funcName = Temp_labelstring(name);
    ...
    if (!isLibFunc) {
        Tr_level current = lv;
        T_exp staticLink = F_staticLinkExp(T_Temp(F_FP()));
