...
static void assemble(const char *filename, const char* obj_name) {
    assert(filename);
    char command[128];
    const char *gcc_compiler_flags = "-m32 -g";
    sprintf(command, "%s -o %s %s runtime.c %s", "gcc ", obj_name ? obj_name : "a.out", gcc_compiler_flags, filename);
    system(command);
}
...
int main(int argc, char **argv) {
    ...
    if (!fs.f_major_s) {
        assemble(outfile, fs.object_name);
    }
